...
use \local_authplugin\forms\appform;
...
$PAGE->set_url('/local/authplugin/subscriptions/manageapps.php', array('id'=>$id));
...
$redirecturl = new moodle_url('/local/authplugin/authplugin_subscription.php', array());

	//handle delete actions
    if($action == 'confirmdelete'){
		$renderer = $PAGE->get_renderer('local_authplugin');
		echo $renderer->header('apps', null, get_string('confirmitemdeletetitle', 'local_authplugin'));
		echo $renderer->confirm(get_string("confirmitemdelete","local_authplugin",$item->appname),
			new moodle_url('/local/authplugin/subscriptions/manageapps.php', array('action'=>'delete','id'=>$id)),
			$redirecturl);
